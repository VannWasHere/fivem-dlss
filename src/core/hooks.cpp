...
HRESULT STDMETHODCALLTYPE Hooks::HookedPresent(
    ...
    // Draw simple overlay using GDI (Works on any D3D11 game)
    if (s_Instance && s_Instance->m_GameWindow) {
        HDC hdc = GetDC(s_Instance->m_GameWindow);
        if (hdc) {
            // Set up text
            ...
            // Create font
            static HFONT hFont = CreateFontA(20, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE,
            ...
            // Draw text
            const char* text = "[FiveM FrameGen] Plugin Loaded - F10: Menu | F9: Toggle";
            TextOutA(hdc, 10, 10, text, (int)strlen(text));
            
            ReleaseDC(s_Instance->m_GameWindow, hdc);
        }
    }
    
    // Set render target and call ImGui callback
    if (s_Instance && s_Instance->m_Device && s_Instance->m_Context && s_Instance->m_RenderTargetView) {
        s_Instance->m_Context->OMSetRenderTargets(1, &s_Instance->m_RenderTargetView, nullptr);
